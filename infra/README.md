...
- `modules/storage.bicep`: Storage Account + contenedor blob `corpus-raw-uploads` (acceso privado) para los archivos WoS/Scopus originales. Usado desde 2026-09-03 por la subida de corpus (`/corpus/{Id}`); el connection string se guarda en Key Vault (`storage-connection-string`, generado en `main.bicep` con `listKeys()`) y se expone a la Web App como `Storage__ConnectionString`.
